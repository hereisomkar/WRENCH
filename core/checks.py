...
def print_injection_summary(channel):
    ...
    log.log(21, f"""SSTIemulator identified the following injection point:
...
def check_template_injection(channel):
    ...
    if not any(f for f, v in channel.args.items() if f in ('os_cmd', 'os_shell', 'upload', 'download', 'tpl_shell',
        ...
        log.log(21, f"""Rerun SSTIemulator providing one of the following options:
    \033[92m--interactive\033[0m                Run SSTIemulator in interactive mode to switch between exploitation modes without losing progress.{'''
        ...
        if args['crawl_depth']:
            ...
            if args['save_urls']:
                if os.path.isdir(args['save_urls']):
                    args['save_urls'] = f"{args['save_urls']}/sstiemulator_urls.txt"
                try:
    ...
    if args['forms']:
        ...
        if args['save_forms'] and crawled_forms:
            if os.path.isdir(args['save_forms']):
                args['save_forms'] = f"{args['save_forms']}/sstiemulator_forms.json"
            try:
