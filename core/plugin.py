...
class Plugin(object):
    ...
    header_type = 'cat'
    sstiemulator_version = config.version
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if module[0] == "plugins":
            if config.compare_versions(cls.sstiemulator_version, config.min_version['plugin']) == "<":
                log.log(22, f'''{cls.__name__} plugin is outdated and cannot be loaded''')
                failed_plugins.append(cls)
                return
            if config.compare_versions(cls.sstiemulator_version, config.version) == ">":
                log.log(22, f'''{cls.__name__} plugin requires SSTIemulator update and cannot be loaded''')
                failed_plugins.append(cls)
                return
