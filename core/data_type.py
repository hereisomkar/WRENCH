...
class DataType(object):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        name = cls.__name__
        if config.compare_versions(cls.sstiemulator_version, config.min_version['data_type']) == "<":
        ...
        if config.compare_versions(cls.sstiemulator_version, config.version) == ">":
            log.log(22, f'''{name} data type requires SSTIemulator update and cannot be loaded''')
            failed_data_types.append(cls)
            return
