...
class InteractiveShell(cmd.Cmd):
    ...
    def __init__(self, args):
        ...
        self.prompt = "SSTIemulator> "
        self.core_prompt = ""
        self.sstiemulator_options = args.copy()
        self.sstiemulator_options.update({"tpl_shell": False, "tpl_cmd": None, "os_shell": False, "os_cmd": None,
        ...
        if self.sstiemulator_options["url"]:
            self.do_url(args.get("url"))
            self.channel = Channel(self.sstiemulator_options)
        if args["load_urls"]:
    ...
    def set_module(self, module):
        ...
        if not self.sstiemulator_options.get("colour", True):
            module = no_colour(module)
        self.prompt = f"SSTIemulator{f' ({module})' if module else ''}> "
...
# Information commands
    ...
    def do_help(self, line):
        log.log(23, """SSTIemulator is an automatic SSTI detection and exploitation tool with predetermined and interactive modes.

SSTIemulator:
  ?, help                                 Show this help message
  version                                 Print SSTIemulator version
  opt, options                            Display current SSTIemulator options
  info                                    Show information about detection results
  reload, reload_modules                  Reload all SSTIemulator plugins and data types
...
Request:
  ...
  log_response                            Toggle including HTTP responses into ~/.sstiemulator/sstiemulator.log
...
Exploitation:
  ...
  down, download [REMOTE] [LOCAL]         Download REMOTE to LOCAL files""")
    ...
    def do_version(self, line):
        """Show current SSTIemulator version"""
        log.log(23, f'Current SSTIemulator version: {self.sstiemulator_options["version"]}')
    ...
    def do_config(self, line):
        if line:
            ...
            if os.path.exists(line):
                ...
                config.config_update(self.sstiemulator_options, custom_config)
                log.log(24, f'Config updated from file: {line}')
                return
        log.log(25, 'Provide file or directory to read config from.')

    def do_options(self, line):
        """Show current SSTIemulator options"""
        crawl_domains = {"Y": "Yes", "S": "Subdomains only", "N": "No"}
        log.log(23, f'Current SSTIemulator {self.sstiemulator_options["version"]} interactive mode options:')
        if not self.sstiemulator_options["url"] and not self.sstiemulator_options["loaded_urls"] \
                and not self.sstiemulator_options["loaded_forms"]:
            log.log(25, f'URL is not set.')
        elif self.sstiemulator_options["loaded_forms"]:
            log.log(26, f'Forms to scan: {len(self.sstiemulator_options["loaded_forms"])}')
            if self.sstiemulator_options["forms"]:
                ulen = 1 if self.sstiemulator_options["url"] else 0
                if self.sstiemulator_options["loaded_urls"]:
                    ulen += len(self.sstiemulator_options["loaded_urls"])
                log.log(26, f'URLs to scan: {ulen}')
        elif self.sstiemulator_options["loaded_urls"]:
            log.log(26, f'URLs to scan: '
                        f'{len(self.sstiemulator_options["loaded_urls"]) + (1 if self.sstiemulator_options["url"] else 0)}')
        else:
            log.log(26, f'URL: {self.sstiemulator_options["url"]}')
        log.log(26, f'Injection marker: {self.sstiemulator_options["marker"]}')
        if self.sstiemulator_options["data"]:
            data = "\n    ".join(self.sstiemulator_options["data"])
            log.log(26, f'Request body data:\n    {data}')
            log.log(26, f'Request body type: {self.sstiemulator_options["data_type"]}')
            if self.sstiemulator_options["data_params"]:
                params = "\n    ".join([f"{x}: {self.sstiemulator_options['data_params'][x]}"
                                        for x in self.sstiemulator_options["data_params"]])
                log.log(26, f'Request body type params:\n    {params}')
        if self.sstiemulator_options["headers"]:
            headers = "\n    ".join(self.sstiemulator_options["headers"])
            log.log(26, f'HTTP headers:\n    {headers}')
        if self.sstiemulator_options["cookies"]:
            cookies = "\n    ".join(self.sstiemulator_options["cookies"])
            log.log(26, f'Cookies:\n    {cookies}')
        log.log(26, f'HTTP method: '
                    f'{self.sstiemulator_options["method"] if self.sstiemulator_options["method"] else "Detect automatically"}')
        if self.sstiemulator_options["random_agent"]:
            log.log(26, 'User-Agent is randomised')
        else:
            log.log(26, f'User-Agent: {self.sstiemulator_options["user_agent"]}')
        if self.sstiemulator_options["delay"]:
            log.log(26, f'Delay between requests: {self.sstiemulator_options["delay"]}s')
        if self.sstiemulator_options["proxy"]:
            log.log(26, f'Proxy: {self.sstiemulator_options["proxy"]}')
        log.log(26, f'Verify SSL: {self.sstiemulator_options["verify_ssl"]}')
        if self.sstiemulator_options["force_level"]:
            log.log(26, f'Forced level: {self.sstiemulator_options["force_level"][0]}')
            log.log(26, f'Forced context level: {self.sstiemulator_options["force_level"][1]}')
        else:
            log.log(26, f'Level: {self.sstiemulator_options["level"]}')
        log.log(26, f'Engine: {self.sstiemulator_options["engine"] if self.sstiemulator_options["engine"] else "*"}'
                    f'{"+" if not self.sstiemulator_options["engine"] and self.sstiemulator_options["legacy"] else ""}'
                    f'{"»" if not self.sstiemulator_options["engine"] and self.sstiemulator_options["skip_generic"] else ""}')
        if self.sstiemulator_options["crawl_depth"] > 0:
            log.log(26, f'Crawler depth: {self.sstiemulator_options["crawl_depth"]}')
            if self.sstiemulator_options["crawl_exclude"]:
                log.log(26, f'Crawler exclude RE: "{self.sstiemulator_options["crawl_exclude"]}"')
            log.log(26, f'Crawl other domains: {crawl_domains.get(self.sstiemulator_options["crawl_domains"].upper())}')
        else:
            log.log(26, 'Crawler: no crawl')
        log.log(26, f'Form detection: {self.sstiemulator_options["forms"]}')
        if self.sstiemulator_options["forms"]:
            log.log(26, f'Allow empty forms: {self.sstiemulator_options["empty_forms"]}')
        log.log(26, f'Attack technique: {self.sstiemulator_options["technique"]}')
        if "T" in self.sstiemulator_options["technique"]:
            log.log(26, f'Time-based blind detection delay: {self.sstiemulator_options["time_based_blind_delay"]}')
            log.log(26, f'Time-based blind verification and exploitation delay: {self.sstiemulator_options["time_based_verify_blind_delay"]}')
        log.log(26, f'Force overwrite files: {self.sstiemulator_options["force_overwrite"]}')
        log.log(26, f'Expected remote shell: {self.sstiemulator_options["remote_shell"]}')
        if self.sstiemulator_options["log_response"]:
            log.log(26, 'HTTP responses will be included into ~/.sstiemulator/sstiemulator.log')
...
# Target commands
    ...
    def do_url(self, line):
        ...
        self.sstiemulator_options["url"] = line
        if not (self.sstiemulator_options['loaded_forms'] or self.sstiemulator_options['loaded_urls']):
    ...
    def do_load_urls(self, line):
        if line:
            ...
            if os.path.exists(line):
                try:
                    with open(line, 'r') as stream:
                        self.sstiemulator_options["loaded_urls"] = set([x.strip() for x in stream.readlines()])
                    log.log(21, f"Loaded {len(self.sstiemulator_options['loaded_urls'])} URL(s) from file: {line}")
                    if not self.sstiemulator_options['loaded_forms']:
                        self.set_module(f"\033[31m{len(self.sstiemulator_options['loaded_urls'])} URLs\033[0m")
                    self.checked = False
            ...
            self.sstiemulator_options["loaded_urls"] = None
            if not self.sstiemulator_options['loaded_forms']:
                self.set_module(f'\033[31m{parse.urlparse(self.sstiemulator_options["url"]).netloc}'
                                f'\033[0m' if self.sstiemulator_options["url"] else "")
    ...
    def do_load_forms(self, line):
        if line:
            ...
            if os.path.exists(line):
                try:
                    with open(line, 'r') as stream:
                        self.sstiemulator_options["loaded_forms"] = set([tuple(x) for x in json.load(stream)])
                    log.log(21, f"Loaded {len(self.sstiemulator_options['loaded_forms'])} forms from file: {line}")
                    self.set_module(f"\033[31m{len(self.sstiemulator_options['loaded_forms'])} forms\033[0m")
                    self.checked = False
                except Exception as e:
            ...
            self.sstiemulator_options["loaded_forms"] = None
            if self.sstiemulator_options['loaded_urls']:
                self.set_module(f"\033[31m{len(self.sstiemulator_options['loaded_urls'])} URLs\033[0m")
            else:
                self.set_module(f'\033[31m{parse.urlparse(self.sstiemulator_options["url"]).netloc}'
    ...
    def do_save_urls(self, line):
        if line:
            if self.sstiemulator_options.get('crawled_urls', None):
                if os.path.isdir(line):
                    line = f"{line}/sstiemulator_urls.txt"
                try:
                    with open(line, 'w') as stream:
                        stream.write("\n".join(self.sstiemulator_options['crawled_urls']))
    ...
    def do_save_forms(self, line):
        if line:
            if self.sstiemulator_options.get('crawled_forms', None):
                if os.path.isdir(line):
                    line = f"{line}/sstiemulator_forms.json"
                try:
                    with open(line, 'w') as stream:
                        json.dump([x for x in self.sstiemulator_options['crawled_forms']], stream, indent=4)
    ...
    def do_crawl(self, line):
        if not line.isnumeric():
            line = "0"
        self.sstiemulator_options['crawl_depth'] = int(line)
        if int(line):
    ...
    def do_exclude(self, line):
        self.sstiemulator_options['crawl_exclude'] = line
        if line:
    ...
    def do_forms(self, line):
        overwrite = not self.sstiemulator_options['forms']
        log.log(24, f'Form detection {"en" if overwrite else "dis"}abled.')
        self.sstiemulator_options['forms'] = overwrite

    def do_empty_forms(self, line):
        overwrite = not self.sstiemulator_options['empty_forms']
        log.log(24, f'Empty form processing {"en" if overwrite else "dis"}abled.')
        self.sstiemulator_options['empty_forms'] = overwrite

    def do_color(self, line):
        colour = not self.sstiemulator_options['colour']
        self.sstiemulator_options['colour'] = colour
    ...
    def do_run(self, line):
        ...
        if not (self.sstiemulator_options["url"] or self.sstiemulator_options["loaded_urls"] or self.sstiemulator_options["loaded_forms"]):
            log.log(22, 'Target URL cannot be empty.')
            return
        try:
            self.current_plugin, self.channel = checks.scan_website(self.sstiemulator_options)
...
# Request commands
    ...
    def do_marker(self, line):
        ...
        self.sstiemulator_options["marker"] = line
    ...
    def do_data(self, line):
        ...
        if line == "":
            ...
            self.sstiemulator_options["data"] = []
            return
        ...
        if (command[0] == "remove" or command[0] == "rm") and len(command) == 2 and command[1] != "":
            ...
            for data in self.sstiemulator_options["data"].copy():
                if data.startswith(command[1]):
                    log.log(26, f'Removing: {data}')
                    self.sstiemulator_options["data"].remove(data)
        else:
            log.log(24, f'Adding request body data: {line}')
            self.sstiemulator_options["data"].append(line)
    ...
    def do_data_params(self, line):
        ...
        if line == "":
            ...
            self.sstiemulator_options["data_params"] = {}
            return
        command = line.split(" ", 1)
        if (command[0] == "remove" or command[0] == "rm") and len(command) == 2 and command[1] != "":
            log.log(24, f'Removing data param {command[1]}:')
            self.sstiemulator_options["data_params"].pop(command[1], None)
        else:
            param = line.split("=", 1)
            log.log(24, f'Adding data param: {param[0]}')
            self.sstiemulator_options["data_params"][param[0]] = param[1]

    def do_header(self, line):
        """Modify HTTP headers"""
        if line == "":
            log.log(24, f'Clearing all HTTP headers...')
            self.sstiemulator_options["headers"] = []
            return
        command = line.split(" ", 1)
        if (command[0] == "remove" or command[0] == "rm") and len(command) == 2 and command[1] != "":
            log.log(24, f'Removing HTTP headers starting with {command[1]}:')
            for header in self.sstiemulator_options["headers"].copy():
                if header.startswith(command[1]):
                    log.log(26, f'Removing: {header}')
                    self.sstiemulator_options["headers"].remove(header)
        else:
            log.log(24, f'Adding HTTP header: {line}')
            self.sstiemulator_options["headers"].append(line)
    ...
    def do_cookie(self, line):
        ...
        if line == "":
            ...
            self.sstiemulator_options["cookies"] = []
            return
        command = line.split(" ", 1)
        if (command[0] == "remove" or command[0] == "rm") and len(command) == 2 and command[1] != "":
            log.log(24, f'Removing cookies starting with {command[1]}:')
            for cookie in self.sstiemulator_options["cookies"].copy():
                if cookie.startswith(command[1]):
                    log.log(26, f'Removing: {cookie}')
                    self.sstiemulator_options["cookies"].remove(cookie)
        else:
            log.log(24, f'Adding cookie: {line}')
            self.sstiemulator_options["cookies"].append(line)
    ...
    def do_http_method(self, line):
        ...
        self.sstiemulator_options["method"] = line
    ...
    def do_data_type(self, line):
        ...
        self.sstiemulator_options["data_type"] = line
    ...
    def do_user_agent(self, line):
        ...
        self.sstiemulator_options["user_agent"] = line

    do_agent = do_user_agent

    def do_random_agent(self, line):
        """Switch random_user_agent option"""
        overwrite = not self.sstiemulator_options["random_agent"]
        log.log(24, f'Value of \'random_user_agent\' is set to {overwrite}')
        self.sstiemulator_options["random_agent"] = overwrite
    ...
    def do_delay(self, line):
        ...
        try:
            self.sstiemulator_options["delay"] = max(float(line), 0)
        except:
            log.log(22, 'Invalid delay time.')
            return
        log.log(24, f'Delay between requests is set to {self.sstiemulator_options["delay"]}')
    ...
    def do_proxy(self, line):
        ...
        if line == "":
            ...
            self.sstiemulator_options["proxy"] = None
            return
        log.log(24, f'Setting proxy to {line}')
        self.sstiemulator_options["proxy"] = line

    def do_verify_ssl(self, line):
        """Switch verify_ssl option"""
        overwrite = not self.sstiemulator_options["verify_ssl"]
        log.log(24, f'Value of \'verify_ssl\' is set to {overwrite}')
        self.sstiemulator_options["verify_ssl"] = overwrite

    do_ssl = do_verify_ssl

    def do_log_response(self, line):
        """Switch log_response option"""
        overwrite = not self.sstiemulator_options["log_response"]
        log.log(24, f'Value of \'log_response\' is set to {overwrite}')
        self.sstiemulator_options["log_response"] = overwrite
...
# Detection commands
    ...
    def do_level(self, line):
        ...
        self.sstiemulator_options["level"] = level
    ...
    def do_force_level(self, line):
        ...
        if line == "":
            ...
            self.sstiemulator_options["force_level"] = None
            return
        line = line.split(" ")
        ...
        self.sstiemulator_options["force_level"] = force_level
    ...
    def do_engine(self, line):
        ...
        self.sstiemulator_options["engine"] = line
    ...
    def do_crawl_domains(self, line):
        ...
        self.sstiemulator_options["crawl_domains"] = line
    ...
    def do_blind_delay(self, line):
        ...
        try:
            self.sstiemulator_options["time_based_blind_delay"] = max(int(line), 1)
        except:
            log.log(22, 'Invalid time-based blind injection delay time.')
            return
        log.log(24, f'Delay for time-based blind injection detection is set to {self.sstiemulator_options["time_based_blind_delay"]}')
    ...
    def do_verify_delay(self, line):
        """Set DELAY for blind SSTI detection"""
        try:
            self.sstiemulator_options["time_based_verify_blind_delay"] = max(int(line), 1)
        except:
            log.log(22, 'Invalid time-based blind injection delay time.')
            return
        log.log(24, f'Delay for time-based blind injection verification and exploitation is set to {self.sstiemulator_options["time_based_verify_blind_delay"]}')

    do_verify_blind_delay = do_verify_delay

    def do_legacy(self, line):
        """Switch legacy option"""
        overwrite = not self.sstiemulator_options["legacy"]
        log.log(24, f'Value of \'legacy\' is set to {overwrite}')
        self.sstiemulator_options["legacy"] = overwrite

    def do_generic(self, line):
        """Switch skip_generic option"""
        overwrite = not self.sstiemulator_options["skip_generic"]
        log.log(24, f'Value of \'skip_generic\' is set to {overwrite}')
        self.sstiemulator_options["skip_generic"] = overwrite
...
# Exploitation commands
    ...
    def do_force_overwrite(self, line):
        ...
        overwrite = not self.sstiemulator_options["force_overwrite"]
        log.log(24, f'Value of \'force_overwrite\' is set to {overwrite}')
        self.sstiemulator_options["force_overwrite"] = overwrite
...
# SSTIemulator commands
    ...
    def do_reload_modules(self, line):
        ...
        from sstiemulator import load_data_types
        unload_data_types()
        load_data_types()
