import argparse
from sstiemulator import version


def banner():
    msg = """\033[93m
                                     \033[42m\033[49m  \033[94m           
███████╗███████╗████████╗ ▀█▀         \033[42m\033[49m  \033[94m                                   
██╔════╝██╔════╝╚══██╔══╝═╗ ╔═          \033[42m\033[49m  \033[94m                                
███████╗███████╗   ██║    ║ ║               \033[42m\033[49m  \033[94m                             
╚════██║╚════██║   ██║    ║ ║                   \033[42m\033[49m  \033[94m                       
...
"""
    msg += f"\n\033[94m[*]\033[0m Version: {version}" \
            f"\n\033[94m[*]\033[0m Author: \033]8;;\007Wrench\033]8;;\007" \
            f"\n\033[34m[*]\033[0m Based on \033]8;;https://github.com/epinna/tplmap\007tplmap\033]8;;\007" \
            f"\n\033[91m[!] LEGAL DISCLAIMER\033[0m: Usage of SSTIemulator for attacking targets without prior mutual " \
...
parser = argparse.ArgumentParser(description='SSTIemulator is an automatic SSTI detection and exploitation tool '
                                                'with predetermined and interactive modes.')
parser.add_argument('-V', '--version', action='version', version=f'SSTIemulator version {version}')
...
target.add_argument("-i", "--interactive", action="store_const", const=True, dest="interactive",
                    help="Run SSTIemulator in interactive mode")
...
request.add_argument("--log-response", action="store_const", const=True, dest="log_response",
                     help="Include HTTP responses into ~/.sstiemulator/sstiemulator.log")
